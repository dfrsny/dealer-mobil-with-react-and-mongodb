...
import { useDeleteNewsMutation, useFetchAllNewsQuery } from '../../../redux/news/newsApi';
import { Link, useNavigate } from 'react-router-dom';

const ManageNews = () => {
    const navigate = useNavigate(); 
    const { data: news, refetch } = useFetchAllNewsQuery();
                                    ...
                                    news && news.map((newsItem, index) => (
                                        <tr key={index}>
                                            ...
                                            <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 space-x-4">
                                                <Link to={`/dashboard/edit-news/${newsItem._id}`} className="font-medium text-indigo-600 hover:text-indigo-700 mr-2 hover:underline underline-offset-2">
                                                    Edit
                                                </Link>
                                                <button 
